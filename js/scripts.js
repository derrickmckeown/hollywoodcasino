...
var $sidebar = $("#masthead"),
    $window = $(window),
    sidebartop = $("#masthead").position().top;
...
if($(window).width() > 768) {	
var baseCheckHeight	= $('.base-check').outerHeight() / 2;
//console.log('base-height '+baseCheckHeight);
$('.give-height').outerHeight(baseCheckHeight);
} else {
$('.give-height').css('height' , 'auto');
}}

function greyHeight() {
   if($(window).width() > 768) {
     var baseCheckGrey = $('.contactbox').outerHeight();  
     $('.greygutter').outerHeight(baseCheckGrey);
   }
}
...
function giveWidth() {
    if($(window).width() > 768) {
        checkWidth = $('.col-wrap').outerWidth();
        $('.give-width').outerWidth(checkWidth / 3);      
    }
}

function giveBannerWidth() {
    if($(window).width() > 768) {
        var totalWidth = 0;
        $('.check-width').each(function(index) {
        totalWidth += parseInt($(this).outerWidth(), 10);
        }); 
        // console.log(totalWidth)
        $('.banner-wrap').outerWidth(totalWidth);
    }
}
